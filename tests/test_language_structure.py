...
def test_all_language_files_exist():
    ...
    assert actual_languages == expected_languages, (
        f"Expected {expected_languages}, got {actual_languages}"
    )
...
def test_all_language_files_have_translations():
    ...
    for modname, module in language_modules:
        assert hasattr(module, "TRANSLATIONS"), (
            f"Language {modname} missing TRANSLATIONS dictionary"
        )
        assert isinstance(module.TRANSLATIONS, dict), (
            f"Language {modname} TRANSLATIONS is not a dict"
        )
...
def test_no_deprecated_entries():
    ...
    for modname, module in language_modules:
        ...
        assert not found_deprecated, (
            f"Language {modname} contains deprecated keys: {found_deprecated}"
        )
    ...
    for modname, module in language_modules:
        for key, value in module.TRANSLATIONS.items():
            assert isinstance(value, str), (
                f"Language {modname} key '{key}' has non-string value: {type(value)}"
            )
            assert value.strip(), (
                f"Language {modname} key '{key}' has empty or whitespace-only value"
            )
...
def test_language_files_have_proper_structure():
    ...
    for modname, module in language_modules:
        ...
        assert "translations" in module.__doc__.lower(), (
            f"Language {modname} docstring doesn't mention translations"
        )

        # Check for __all__ export
        assert hasattr(module, "__all__"), f"Language {modname} missing __all__ export"
        assert "TRANSLATIONS" in module.__all__, (
            f"Language {modname} __all__ doesn't include TRANSLATIONS"
        )
...
def test_verification_states_consistency():
    ...
    for modname, module in language_modules:
        ...
        assert verified != not_verified, (
            f"Language {modname} has same translation for VERIFIED and NOT_VERIFIED"
        )
    ...
    for modname, module in language_modules:
        ...
        for key, value in module.TRANSLATIONS.items():
            if value in value_to_keys:
                ...
                if (key, existing_key) not in allowed_duplicates and (
                    existing_key,
                    key,
