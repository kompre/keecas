...
def enable_pint_locale():
    ...
    config.language.disable_pint_locale = False
    yield
    config.language.disable_pint_locale = original_setting
...
def test_available_languages():
    ...
    assert len(languages) == len(expected_languages), (
        f"Expected {len(expected_languages)} languages, got {len(languages)}: {languages}"
    )
    assert set(languages) == set(expected_languages), (
        f"Language set mismatch. Expected: {expected_languages}, Got: {languages}"
    )
...
def test_toml_config_custom_replacements():
    ...
    try:
        ...
        with patch(
            "keecas.localization.get_custom_replacements_from_config",
...
def test_no_config_file():
    ...
    try:
        ...
        with (
            patch("keecas.localization.get_language_from_config", return_value=None),
...
def test_new_language_translations():
    ...
    for lang_code, translations in expected_translations.items():
        ...
        assert translate("VERIFIED") == translations["VERIFIED"], (
            f"VERIFIED translation failed for {lang_code}"
        )
        assert translate("NOT_VERIFIED") == translations["NOT_VERIFIED"], (
            f"NOT_VERIFIED translation failed for {lang_code}"
        )

        # Test SymPy words
        assert translate("for") == translations["for"], f"'for' translation failed for {lang_code}"
        assert translate("otherwise") == translations["otherwise"], (
            f"'otherwise' translation failed for {lang_code}"
        )
...
def test_check_function_multilingual():
    ...
    for lang_code, verified_term, not_verified_term in test_cases:
        ...
        # Test passing case
        result_pass = check(5, 10, Le)
        assert verified_term in result_pass.data, (
            f"Passing check failed for {lang_code}: expected '{verified_term}' in '{result_pass.data}'"
        )

        # Test failing case
        result_fail = check(15, 10, Le)
        assert not_verified_term in result_fail.data, (
            f"Failing check failed for {lang_code}: expected '{not_verified_term}' in '{result_fail.data}'"
        )
...
def test_show_eqn_language_replacements():
    ...
    for lang_code, for_word, otherwise_word in test_cases:
        ...
        assert rf"\text{{{for_word}}}" in result.data, (
            f"'{for_word}' not found for {lang_code} in: {result.data}"
        )
        assert rf"\text{{{otherwise_word}}}" in result.data, (
            f"'{otherwise_word}' not found for {lang_code} in: {result.data}"
        )
        ...
        assert r"\text{otherwise}" not in result.data, (
            f"English 'otherwise' still present for {lang_code}"
        )
...
def test_pint_locale_initialization():
    ...
    original_mode = config.language.pint_language_mode
    ...
    try:
        ...
        config.language.disable_pint_locale = original_disable
        config.language.pint_language_mode = original_mode
    ...
    try:
        ...
        for lang_code, expected_locale_prefix in test_locales.items():
            ...
            assert actual_locale.startswith(expected_locale_prefix), (
                f"Expected locale starting with '{expected_locale_prefix}', got {actual_locale}"
            )
...
def test_options_language_auto_sync():
    ...
    try:
        ...
        for lang_code, expected_locale_prefix in test_cases:
            config.language.language = lang_code
            actual_locale = u.formatter.locale
            assert actual_locale.startswith(expected_locale_prefix), (
                f"Auto-sync failed: expected locale starting with '{expected_locale_prefix}', got {actual_locale} for language '{lang_code}'"
            )
        ...
        assert current_locale.startswith("en_"), (
            f"Setting 'en' should reset to English from German, got {current_locale}"
        )

        # Test conservative behavior: 'en' after 'en' should not change
        config.language.language = "en"
        assert u.formatter.locale == current_locale, (
            "Setting 'en' again should be conservative, but it changed"
        )
        ...
        config.language.disable_pint_locale = original_setting
        config.language.language = None
    assert u.formatter.locale == current_locale, "Setting None should not change locale"
...
def test_pint_locale_fallback_behavior(enable_pint_locale):
    ...
    assert "centimeter" in swedish_result.lower(), (
        f"Expected English after Swedish, got: {swedish_result}"
    )
    ...
    for lang in unsupported_langs:
        ...
        assert "centimeter" in result.lower(), (
            f"Language {lang} should fallback to English, got: {result}"
        )
...
def test_pint_locale_supported_languages(enable_pint_locale):
    ...
    for lang, expected_unit in expected_translations.items():
        ...
        assert expected_unit in result, (
            f"Language {lang} should show '{expected_unit}', got: {result}"
        )
