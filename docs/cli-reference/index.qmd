...
### Install the keecas-notebook Skill

Install the bundled `keecas-notebook` skill so Claude Code automatically applies it when authoring calculation notebooks:

```bash
keecas skill install

# Overwrite an existing installation (e.g. after upgrading keecas)
keecas skill install --force
```

This installs into `~/.claude/skills/keecas-notebook/`. Where the platform supports it, the skill is symlinked to the version bundled with your installed `keecas` package, so it updates automatically on `pip install -U keecas`; otherwise (e.g. Windows without symlink support) it falls back to a plain copy, and you should re-run `keecas skill install --force` after upgrading keecas. A stale install from a previous keecas version's legacy location is removed automatically. Restart Claude Code (or open a new session) after installing.

### Use the Skill with Other AI Tools

For AI tools that don't understand Claude Code's Skill format (OpenAI Codex CLI, Cursor, Copilot, or a plain chat session), print the skill content to stdout instead of installing it, and paste or redirect it wherever you need:

```bash
# Print the main skill guidance
keecas skill print

# Include the skill's reference docs too (roughly double the size)
keecas skill print --with-references

# e.g. append it to a project-level AGENTS.md
keecas skill print --with-references >> AGENTS.md
```

This makes no filesystem writes on its own -- it's plain text on stdout, so it works the same on every OS and with any tool that accepts pasted or piped context.
...
- Check the [example notebooks](https://github.com/kompre/keecas/tree/main/examples) for workflow patterns
